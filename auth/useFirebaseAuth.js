...
import {
  getGreenBusOnBoardFromCookie,
  removeGreenBusOnBoardCookie,
  setGreenBusOnBoardCookie,
} from "./userCookies";
...
export default function useFirebaseAuth() {
  ...
  const createUserWithEmailMethod = async (email, password) => {
    try {
      ...
      const error = firebaseErrorFinder[e.code]
        ...
        : "Somthing went wrong during authentication please refresh and retry";
      toast.error(error, {
  ...
  const loginWithEmailAndPassword = async (email, password, redirect = "") => {
    try {
      ...
      // to also get fresh firebase token
  ...
  const deleteMyAccount = async (email, password) => {
    try {
