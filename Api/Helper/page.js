...
console.log("API base URL is:", process.env.NEXT_PUBLIC_API_BASE_URL);
...
export function uploadImageLocally(file) {
  // generate a universally-unique key
  const key =
    crypto.randomUUID?.() ??
    "img_" + Math.random().toString(36).substring(2, 10) + "_" + Date.now();
...
export async function uploadFile(file) {
  ...
  formData.append("type", "kamelion");
  formData.append("file", file);

  const res = await fetch(url + `/users/fileupload`, {
    method: "POST",
    headers: {
